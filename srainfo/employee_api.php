...
switch ($action) {
    ...
    case 'add_employee':
        ...
        if ($conn->query($sql)) ok(['id' => $conn->insert_id]);
    ...
    case 'delete_employee':
        ...
        $id = intval($b['id']);
        if (!$id) fail('Invalid ID');
        if ($conn->query("DELETE FROM employees WHERE id=$id")) ok();
        else fail($conn->error);
        break;
    ...
    case 'delete_unemployed':
        ...
        $id = intval($b['id']);
        if (!$id) fail('Invalid ID');
        if ($conn->query("DELETE FROM unemployed_employees WHERE id=$id")) ok();
        else fail($conn->error);
        break;
    ...
    case 'delete_contract':
        ...
        $id = intval($b['id']);
        if (!$id) fail('Invalid ID');
        if ($conn->query("DELETE FROM employee_contracts WHERE id=$id")) ok();
        else fail($conn->error);
        break;
